...
#   @impl true
#   def init(_opts) do
#     config = Application.get_env(:najva, __MODULE__, [])
#     jid_str = Keyword.fetch!(config, :jid)
#     password = Keyword.fetch!(config, :password)
#     host = Keyword.fetch!(config, :host)
#     port = Keyword.get(config, :port, 5222)
#
...
#       starttls_proceed() when state.connection_state == :starttls_sent ->
...
#         {:ok, tls_socket} = :ssl.connect(state.socket, [{:active, :once}, {:verify, :verify_none}])
...
#       iq(type: :result, sub_els: [bind(jid: full_jid)]) ->
#         Logger.info("Resource bound successfully: #{jid(full_jid) |> :jid.to_string() |> to_string()}")
...
#       message(from: from, sub_els: sub_els) ->
...
#     sasl_mechanisms = :xmpp.get_subtag(stream_features_element, :mechanisms)
#     # We'll use PLAIN for simplicity. SCRAM is more secure.
#     if "PLAIN" in sasl_mechanisms.mechanisms do
...
#       {:noreply, state}
#     else
#       Logger.error("Server does not support PLAIN authentication.")
...
#     bind_el = bind()
#     iq_el = iq(type: :set, id: :crypto.strong_rand_bytes(8) |> Base.encode16(), sub_els: [bind_el])
