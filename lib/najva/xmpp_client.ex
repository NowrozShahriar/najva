defmodule Najva.XmppClient do
	...
	def init(opts) do
		...
		state = %{
			...
			stream_state: :fxml_stream.new(self()),
			socket: nil
		}
	...
	@impl true
	def handle_info({:xmlstreamelement, element}, state),
		do: handle_element(element, state)
		...
		case element do
			...
			element when state.connection_state == :starttls_sent ->
				case element do
					starttls_proceed() ->
						Logger.info(
							"Najva.XmppClient: STARTTLS proceed received, upgrading to TLS (test: verify_none)"
						)
					...
					other ->
						Logger.warning(
							"Najva.XmppClient: unexpected while waiting starttls: #{inspect(other)}"
						)
						{:noreply, state}
				end
			...
			sasl_success() ->
				...
				new_state = %{state | stream_state: :fxml_stream.new(self(), state.host, [])}
				send_stream_header(new_state)
				{:noreply, new_state}
		...
		case :xmpp.get_subtag(stream_features_element, :mechanisms) do
			sasl_mechanisms(list: mechanisms) when is_list(mechanisms) ->
				...
				if Enum.member?(mech_strings, "PLAIN") do
					...
					Logger.error(
						"Najva.XmppClient: server does not support PLAIN: #{inspect(mech_strings)}"
					)
					{:stop, :no_plain, state}
				end
