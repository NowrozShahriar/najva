defmodule Najva.Accounts do
  ...
  See `Najva.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `Najva.Accounts.User.password_changeset/3` for a list of supported options.
  ...
  def delete_user_session_token(token) do
    Repo.delete_all(
      from(t in UserToken, where: t.token == ^token and t.context in ["session", "one_time"])
    )
